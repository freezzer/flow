// @Generated(server/src/praxis/bom/Dokument.java)
/* 

           generated code by flow "flex on wings"
           this code is property of Andreas Marochow
    
*/ 

package praxis.bom;
import de.ama.framework.data.*;
import java.math.BigDecimal;

public class Dokument  implements java.io.Serializable , de.ama.db.PersistentMarker { 


    private String typ;
    public  String getTyp() { return de.ama.util.Util.saveToString(typ); }
    public  void   setTyp(String in) {  typ=in; }

    private String name;
    public  String getName() { return de.ama.util.Util.saveToString(name); }
    public  void   setName(String in) {  name=in; }

    private String url;
    public  String getUrl() { return de.ama.util.Util.saveToString(url); }
    public  void   setUrl(String in) {  url=in; }




// *****************************************************************************************
// [MANUAL.CODE] insert manual code here
// *****************************************************************************************
// [MANUAL.CODE.END] 

}
