package de.ama.services.impl;

import de.ama.db.Query;
import de.ama.framework.util.ImageConverter;
import de.ama.huyua.Picture;
import de.ama.huyua.Voting;
import de.ama.huyua.VotingSum;
import de.ama.services.DomainService;
import de.ama.services.Environment;
import de.ama.util.Ini;
import de.ama.util.Log;
import de.ama.util.Util;
import org.apache.commons.fileupload.FileItem;

import java.io.File;
import java.util.ArrayList;
import java.util.List;

/**
 * Created by IntelliJ IDEA.
 * User: x
 * Date: 04.06.2009
 * Time: 23:04:05
 * To change this template use File | Settings | File Templates.
 */
public class DomainServiceImpl implements DomainService {
    public static String PICTURE_NUMBER = "picture_number";

    public DomainServiceImpl() {
        Environment.getPersistentService().createSequenze(PICTURE_NUMBER);
    }

    public void storePicture(User user, FileItem fileItem, String description){
        Picture picture = new Picture();
        picture.setOwner(user);
        picture.setDescription(description);
        String context = Ini.getString("server.context","/huyua"       ,"http context/doc-base of this server");

        long num = Environment.getPersistentService().getNextNumber(PICTURE_NUMBER);
        String fname = ""+ num +"_"+fileItem.getName();
        File file = de.ama.util.Environment.getFile("data/"+context+"/public/"+user.getName() , fname);

        String thumbName = "Thumb_"+num+"_"+fileItem.getName();
        File thumb = de.ama.util.Environment.getFile("data/"+context+"/public/"+user.getName() , thumbName);

        picture.setFileName("public/"+user.getName()+"/"+fname);
        picture.setThumbName("public/"+user.getName()+"/"+thumbName);

        try {
            ImageConverter.writeCompressedAndScaledJpeg(fileItem.getInputStream() , file, 800,800, 1f);
            Log.write("stored picture "+picture.getFileName());

            ImageConverter.writeCompressedAndScaledJpeg(fileItem.getInputStream() , thumb, 100,100, 0.9f);
            Log.write("stored thumb "+picture.getFileName());

        } catch (Exception e) {
            throw new RuntimeException("storePiture failed : ",e);
        }
        Environment.getPersistentService().makePersistent(picture);
    }

    public void deletePicture(String pictureId) {
        Picture picture = (Picture) Environment.getPersistentService().getObject(pictureId);
        if(picture == null){     return ;  }  // =================================>

        List votings = getVotingsOfPicture(picture);
        Environment.getPersistentService().delete(votings);
        String context = Ini.getString("server.context","/huyua"       ,"http context/doc-base of this server");

        Log.write("deleted picture "+picture.getFileName()+" and ("+votings.size()+") votings");
        File file = de.ama.util.Environment.getFile("data/"+context ,picture.getFileName() );
        if(file.exists()){
            file.delete();
        }
        File thumb = de.ama.util.Environment.getFile("data/"+context ,picture.getThumbName() );
        if(thumb.exists()){
            thumb.delete();
        }

        Environment.getPersistentService().delete(picture);
    }


    public List findPictures(User user) {
        Query q = new Query(Picture.class, "owner" , Query.EQ, user);
        List ret = Environment.getPersistentService().getObjects(q);
        return ret;
    }

    public List findVotedPictures(User user) {
        Query q = new Query(Voting.class, "user" , Query.EQ, user);
        List votings = Environment.getPersistentService().getObjects(q);
        ArrayList ret = new ArrayList();
        ArrayList garbage = new ArrayList();
        for (int i = 0; i < votings.size(); i++) {
            Voting voting = (Voting) votings.get(i);
            try {

                Picture picture = voting.getPicture();
                if( Util.isEmpty(picture.getFileName())
                 && picture.getVoteCount() <= 0) {
                    garbage.add(voting);
                    garbage.add(picture);
                } else {
                    if(!ret.contains(picture)){
                        ret.add(picture);
                    }
                }
            } catch (Exception e) {
                garbage.add(voting);
            }
        }

        if(garbage.size()>0){
            Log.write("delete garbage objects ("+garbage.size()+ ")");
            Environment.getPersistentService().delete(garbage);
        }

        return ret;
    }


    public Picture findPicture(String id) {
        if(Util.isEmpty(id)) return null;
        return (Picture) Environment.getPersistentService().getObject(id);
    }

    public Picture findPictureByThumb(String thumbName) {
        if(Util.isEmpty(thumbName)) return null;
        return (Picture) Environment.getPersistentService().getObject(new Query(Picture.class,"thumbName",Query.EQ, thumbName),false);
    }

    public List findNextPicturesToVote(User user, int limit) {
        Query uq = new Query(Picture.class, "owner" , Query.EQ, user);
        Query q = new Query(Picture.class).orderByColumn("voteCount").limit(limit);
        return Environment.getPersistentService().getObjects(q.and(uq.negate()));
    }

    public List findTeasePictures(int limit, String lastOidString) {
        long oid = 0;
        if(Util.isNotEmpty(lastOidString) && lastOidString.indexOf(":")>=0){
            String s = Util.saveToString(lastOidString).split(":")[1];
            if(s !=null){
                oid = Long.parseLong(s);
            }
        }

        Query q  = new Query(Picture.class,"oid",Query.GT,oid).orderByColumn("oid").limit(limit);
        List ret = Environment.getPersistentService().getObjects(q);
        if(ret.size()==limit){
            System.out.println("DomainServiceImpl.findTeasePictures tease 3 new ones");
           return ret;
        }

        System.out.println("DomainServiceImpl.findTeasePictures ooooh");
        return Environment.getPersistentService().getObjects(new Query(Picture.class).limit(limit));

    }


    /////////////////////////////////////// Voting ///////////////////////////////////////

    public Voting findVoting(String id) {
        if(Util.isEmpty(id)) return null;
        return (Voting) Environment.getPersistentService().getObject(id);
    }

    public List findVotingsOfUser(User user) {
        Query q = new Query(Voting.class, "user" , Query.EQ, user);
        return Environment.getPersistentService().getObjects(q);
    }

    public void deleteVoting(String id) {
        Environment.getPersistentService().delete(id);
    }

    public void storeVoting(Voting voting, User user) {
        voting.setUser(user);
        if (voting.getPicture() == null) {
            throw new RuntimeException("Voting without Picture is not allowed");
        }

        Picture picture = findPictureByThumb(voting.getPicture().getThumbName());
        if (picture != null) {
            voting.setPicture(picture);
        }

        Environment.getPersistentService().makePersistent(voting);
    }

    public List getVotingsOfPicture(Picture picture) {
        Query q = new Query(Voting.class, "picture" , Query.EQ, picture);
        return Environment.getPersistentService().getObjects(q);
    }

    public List getVotingsOfPictureAndUser(Picture picture, User user) {
        Query q1 = new Query(Voting.class, "picture" , Query.EQ, picture);
        Query q2 = new Query(Voting.class, "user" , Query.EQ, user);
        return Environment.getPersistentService().getObjects(q1.and(q2));
    }


    public void recalcVotingSum(Picture pic){
        VotingSum votingSum = pic.getVotingSum();
//        if(votingSum==null){
//            votingSum=new VotingSum();
//            pic.setVotingSum(votingSum);
//        }

        votingSum.reset();

        List votings = getVotingsOfPicture(pic);
        for (int i = 0; i < votings.size(); i++) {
            Voting voting = (Voting) votings.get(i);
            votingSum.addVoting(voting);
        }

        votingSum.resume();
        pic.setVoteCount(votings.size());
    }

}
